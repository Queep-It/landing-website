...
import QueepMark from "../../app/components/QueepMark.vue"

describe("QueepMark", () => {
   // The mark sits beside the word "Queep" everywhere it appears, so an
   // accessible name here makes a screen reader read the brand twice.
   it("is hidden from assistive technology", async() => {
      const mark = await mountSuspended(QueepMark)
