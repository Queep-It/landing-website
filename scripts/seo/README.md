...
- `llms.txt` is served as `text/plain` and lists the features and pricing
- for each route in `$ROUTES` (default: all four indexable pages): `<title>`, meta description,
...
ROUTES="/ /contact" scripts/seo/verify.sh                     # check more routes
...
URLS="/ /contact" scripts/seo/lighthouse.sh       # audit multiple routes
```
