#!/usr/bin/env bash
#
# SEO verification smoke test for the Queepit landing site (Nuxt SEO suite).
#
...
# Every indexable route. Four of them, written out rather than derived:
# unlike a tool registry there is nothing to derive them from, and a
# landing site's page list changes about twice a year.
#
# When a page is added, add it here — otherwise its metadata is never
# asserted, which is exactly how a page ships with the site-wide fallback
# title and nobody notices for a month.
ROUTES="${ROUTES:-/ /contact /privacy-policy /terms}"
SERVER_PID=""
...
# model what the site offers. Generated from the content registries, so the
# check that matters is that a feature actually appears — a route that 200s
# with an empty body would otherwise look fine.
section "llms.txt"
assert_contains "starts with an H1"     "$LLMS" "^# .+"
assert_contains "carries a summary"     "$LLMS" "^> .+"
assert_contains "links absolutely"      "$LLMS" "^- \[[^]]+\]\(https?://[^)]+\): .+"
assert_contains "lists the pricing"     "$LLMS" "^## Pricing$"
assert_contains "lists the features"    "$LLMS" "^## Features$"
...
# Two separate blocks, because Google reads them independently: the app
# entity earns the price/platform rich result, the FAQ can surface an
# individual answer. Nesting the FAQ inside the app entity makes it
# ineligible, and nothing else in the pipeline would notice.
